...
var GenerateTabbedHTMLfromArray = function(largeimages, thumbimages, dsbrowblocklabel) {
   ...
   var cke_dialog_body = document.getElementById("cke_dialog_body");
   
   
   //TODO CORRIGER CA
   if(cke_dialog_body != null)
   {
    var link = cke_dialog_body.getElementsByTagName("a");

    for(i=0; i < link.length; i++)
    {
        console.log(link[i]);

        if(link[i].id.indexOf('cke_TabbedImageBrowser') > -1)
        {

            // On quiite la fonction si l'onglet est dejà présent
            return;
        }
    }
   }
...
var GenerateTabbedbrowserHTML = function() {
    ...
    tabbedimages_html += eval(CKEDITOR.getApp() + ".getUploadButton()");
...
spUpload +="<br>";
spUpload +="<input id='hdApp' type='hidden' value='EeBlog'>";
spUpload +="<input id='hdIdElement' type='hidden' value='1'>";
spUpload +="<input id='hdAction' type='hidden' value='UploadImageBlog'>";
spUpload +="<input id='fileToUpload' class='input' type='file' name='fileToUpload' size='45'>";
spUpload +="<button id='buttonUpload' class='cke_dialog_ui_button' onclick='EeBlogAction.upload();'>Envoyer</button>";
spUpload +="</span>";
    
    tabbedimages_html += spUpload;*/
...
CKEDITOR.plugins.add('tabbedimagebrowser',
{
    init: function(editor)
    {
        ...
        CKEDITOR.on('dialogDefinition', function(ev)
        {
            ...
            if (dialogName === 'image')
            {       
                 var tabbedimages_html = "<input type='button'>lkjljlkj</input>";
                         
                         
                   tabbedimages_html +=  GenerateTabbedbrowserHTML();
