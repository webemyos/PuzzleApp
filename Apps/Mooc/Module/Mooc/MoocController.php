 ...
 class MoocController extends Controller
 {
        ...
        function ShowAddMooc($moocId)
        {
            ...
            $jbMooc->AddControls(array(
                                          ...
                                          array("Type"=>"EntityListBox", "Name"=> "lstCategory", "Libelle"=>$this->Core->GetCode("Category"), "Entity"=>"MoocCategory", "Value" => ( ($moocId != "") ? $mooc->CategoryId->Value : "") ),
        ...
        {
          $modele = new View(Mooc::$Directory . "/View/EditMooc.tpl", $this->Core); 
        ...
        function ShowAddLesson($moocId, $lessonId)
        {
            ...
            $jbLesson->AddControls(array(
                                          ...
                                          array("Type"=>"Button", "CssClass" => "btn btn-primary", "Name"=> "BtnSave" , "Value" => $this->Core->GetCode("Save"), "OnClick" => "MoocAction.SaveLesson(".$moocId.", ".$lessonId.")"),
                              )
                    );


            return $jbLesson->Show() . $this->GetElements($lessonId);
        }
        ...
        function StartMooc($moocId, $front = false)
        {
            if($front)
            {
                $directory = str_replace("../Apps", "Apps", Mooc::$Directory);
            }
            else
            {
                $directory = Mooc::$Directory;
            }
